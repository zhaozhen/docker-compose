...
docker exec -i my-build bash -c "cd /opt/jar/ && sh service.sh $branch_name" 
...
### 删除镜像
docker rmi service:$branch_name
### 重新编译镜像
cd /opt/release/service-$branch_name && docker build -t  service:$branch_name . 
### 运行镜像
docker run -itd  --name service-$branch_name  -p 8866:8866 service:$branch_name
