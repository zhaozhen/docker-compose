...
docker exec -i my-build bash -c "cd /opt/jar/ && sh service-batch.sh $branch_name" 
...
### 删除镜像
docker rmi service-batch:$branch_name
### 重新编译镜像
cd /opt/release/service-batch-$branch_name && docker build -t  service-batch:$branch_name . 
### 运行镜像
docker run -itd  --name service-batch-$branch_name  -p 8090:8090 service-batch:$branch_name
